import numpy as np
try:
  from csci3202.im2col_cython import col2im_cython, im2col_cython
  from csci3202.im2col_cython import col2im_6d_cython
except ImportError:
  print('run the following from the csci3202 directory and try again:')
...
def conv_forward_strides(x, w, b, conv_param):
  ...
  shape = (int(C), int(HH), int(WW), int(N), int(out_h), int(out_w))
...
def max_pool_forward_reshape(x, pool_param):
  ...
  x_reshaped = x.reshape(int(N, C, H / pool_height), int(pool_height),
